...
SELECT COUNT(*) INTO v_dup_names
FROM (
         SELECT UPPER(TRIM(LCLS_SYSTM3_NAME)) AS NM
...
MERGE INTO ACCOMMODATION a
  USING (
    ...
    WHERE a.ACCOMMODATION_TYPE IS NOT NULL
      AND UPPER(TRIM(tr.LCLS_SYSTM3_NAME)) = UPPER(TRIM(a.ACCOMMODATION_TYPE))
  ) m
  ON (a.ID = m.AID)
...
MERGE INTO TOUR t
  USING (
    ...
    WHERE t.TOUR_TYPE IS NOT NULL
      AND UPPER(TRIM(tr.LCLS_SYSTM3_NAME)) = UPPER(TRIM(t.TOUR_TYPE))
  ) m
  ON (t.ID = m.TID)
...
MERGE INTO RESTAURANT r
  USING (
    ...
    WHERE r.RESTAURANT_TYPE IS NOT NULL
      AND UPPER(TRIM(tr.LCLS_SYSTM3_NAME)) = UPPER(TRIM(r.RESTAURANT_TYPE))
  ) m
  ON (r.ID = m.RID)
...
WHERE owner = USER AND table_name = 'ACCOMMODATION' AND constraint_name = 'FK_ACCOMMODATION_CATEGORY';
IF v_cnt = 0 THEN
EXECUTE IMMEDIATE
...
SELECT COUNT(*) INTO v_cnt
FROM ALL_CONSTRAINTS
WHERE owner = USER AND table_name = 'TOUR' AND constraint_name = 'FK_TOUR_CATEGORY';
IF v_cnt = 0 THEN
EXECUTE IMMEDIATE
    'ALTER TABLE TOUR '||
...
SELECT COUNT(*) INTO v_cnt
FROM ALL_CONSTRAINTS
WHERE owner = USER AND table_name = 'RESTAURANT' AND constraint_name = 'FK_RESTAURANT_CATEGORY';
IF v_cnt = 0 THEN
EXECUTE IMMEDIATE
