...
WHERE OWNER = USER AND TABLE_NAME = 'PLACE_LCLS_CATEGORY_TR';
IF v_cnt = 0 THEN
    RAISE_APPLICATION_ERROR(-20960, 'PLACE_LCLS_CATEGORY_TR not found.');
END IF;

SELECT COUNT(*) INTO v_cnt FROM ALL_TABLES
WHERE OWNER = USER AND TABLE_NAME = 'PLACE_LCLS_CATEGORY';
IF v_cnt = 0 THEN
...
EXECUTE IMMEDIATE
    'ALTER TABLE PLACE_LCLS_CATEGORY_TR '||
...
BEGIN
EXECUTE IMMEDIATE
    'CREATE INDEX IX_PLLCTR_LOCALE_L3NAME_UPPER '||
